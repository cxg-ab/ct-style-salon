...
import GuestAppointments from '@/pages/guest-appointments';
...
function Shell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="grain min-h-[100dvh]">
      <a href="#main-content" className="sr-only focus:not-sr-only focus:absolute focus:left-4 focus:top-3 focus:z-50 focus:rounded-full focus:bg-[hsl(var(--primary))] focus:px-4 focus:py-2 focus:text-[hsl(var(--primary-foreground))]">{t('skipToContent')}</a>
      <header className="sticky top-0 z-40 border-b border-[hsl(var(--border)/.72)] bg-[hsl(var(--background)/.91)] backdrop-blur-xl">
        <div className="mx-auto flex h-[76px] max-w-[1240px] items-center justify-between px-5 sm:px-8">
          <Link href="/" className="group flex items-center gap-3" data-testid="link-brand">
           ...
           <button className="rounded-full p-2 text-[hsl(var(--foreground))] md:hidden" onClick={() => setMobileOpen(!mobileOpen)} aria-expanded={mobileOpen} aria-controls="mobile-nav" aria-label={mobileOpen ? t('closeMenu') : t('openMenu')} data-testid="button-mobile-menu">
            {mobileOpen ? <X size={22} /> : <Menu size={22} />}
          </button>
        </div>
        {mobileOpen && (
           <nav id="mobile-nav" className="border-t border-[hsl(var(--border)/.72)] bg-[hsl(var(--background))] px-5 py-4 md:hidden" aria-label={t('theSalon')}>
            {links.map((link) => (
...
function bookingFailureMessage(error: unknown, t: (key: 'bookingTaken' | 'bookingErrorGeneric' | 'bookingErrorValidation' | 'bookingErrorUnavailable') => string) {
  const data = (error as { data?: { error?: string } } | undefined)?.data;
  const message = data?.error ?? (error instanceof Error ? error.message : '');
  if (message.includes('just booked') || message.includes('just taken')) return t('bookingTaken');
  if (message.includes('Check your booking')) return t('bookingErrorValidation');
  if (message.includes('not scheduled') || message.includes('no longer available') || message.includes('break')) return t('bookingErrorUnavailable');
  return t('bookingErrorGeneric');
}

function Field({ icon, label, value, onChange, type = 'text', required = false, testId, autoComplete }: { icon: React.ReactNode; label: string; value: string; onChange: (value: string) => void; type?: string; required?: boolean; testId: string; autoComplete?: string }) {
  return <label className="block text-xs font-semibold">{label}<span className="relative mt-2 block"><span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-[hsl(var(--primary))]">{icon}</span><input type={type} required={required} autoComplete={autoComplete} value={value} onChange={(event) => onChange(event.target.value)} className="h-12 w-full rounded-xl border border-[hsl(var(--input))] bg-[hsl(var(--card))] pl-11 pr-4 text-sm font-normal placeholder:text-[hsl(var(--muted-foreground))]" data-testid={testId} /></span></label>;
}
