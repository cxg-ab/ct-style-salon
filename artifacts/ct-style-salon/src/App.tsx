...
function Book() {
  ...
  const servicesQuery = useListServices({ query: { queryKey: getListServicesQueryKey() } });
  const stylistsQuery = useListStylists({ query: { queryKey: getListStylistsQueryKey() } });
  const availabilityParams = useMemo(() => ({ date }), [date]);
  const availabilityQuery = useGetAvailability(availabilityParams, { query: { enabled: Boolean(date), queryKey: getGetAvailabilityQueryKey(availabilityParams) } });
  const createAppointment = useCreateAppointment();
  const services = servicesQuery.data ?? [];
  const stylists = stylistsQuery.data ?? [];
  const selectedService = services.find((service) => service.id === serviceId);
  const selectedStylist = stylists.find((stylist) => stylist.id === stylistId);
  const slots = availabilityQuery.data?.find((availability) => availability.stylistId === stylistId)?.slots ?? [];

  const canNext = (step === 1 && Boolean(serviceId)) || (step === 2 && Boolean(stylistId)) || (step === 3 && Boolean(time)) || step === 4;
