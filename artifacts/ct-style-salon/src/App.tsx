...
function Book() {
  ...
  const displayedService = selectedService ? serviceCopy(selectedService) : undefined;
    ...
    <main className="mx-auto max-w-[1240px] px-5 py-12 sm:px-8 md:py-20">
      ...
      <div className="grid gap-10 lg:grid-cols-[1fr_340px]">
        <div>
         ...
         <aside className="h-fit rounded-2xl bg-[hsl(var(--secondary))] p-6 text-[hsl(var(--card))] lg:sticky lg:top-28"><p className="font-mono-ui text-[10px] uppercase tracking-[.2em] text-[hsl(var(--accent))]">{t('yourVisit')}</p><div className="mt-8 border-b border-[hsl(var(--card)/.15)] pb-6"><p className="font-display text-3xl">{displayedService?.name ?? t('selectService')}</p><p className="mt-2 text-sm text-[hsl(var(--card)/.58)]">{selectedService ? `${selectedService.durationMinutes} ${t('minutes')} · ${formatPrice(selectedService.price)}` : t('ritualBegins')}</p></div><div className="space-y-5 py-6 text-sm"><div className="flex gap-3"><UserRound size={16} className="mt-0.5 text-[hsl(var(--accent))]" /><span>{displayedStylist?.name ?? t('stylistToChoose')}</span></div><div className="flex gap-3"><CalendarDays size={16} className="mt-0.5 text-[hsl(var(--accent))]" /><span>{date ? formatDate(date, { weekday: 'long', month: 'long', day: 'numeric' }) : t('dateToChoose')}{time ? ` · ${time}` : ''}</span></div></div><div className="flex gap-2 border-t border-[hsl(var(--card)/.15)] pt-5 text-[11px] leading-5 text-[hsl(var(--card)/.54)]"><ShieldCheck size={15} className="shrink-0 text-[hsl(var(--accent))]" /> {t('noPayment')}</div></aside>
      </div>
    </main>
  );
}
